...
1. Definir el tag de los tipos de tests que se van a ejecutar, esto lo hacemos en el archivo WebRunnerTest, para el
...
```bash
  ./gradlew clean test --tests "runners.automationtest.WebRunnerTest"
```

Para ejecutar escenarios que contengan un tag especifico

```bash
  ./gradlew clean test --tests -Dcucumber.filter.tags="@test" runners.automationtest.WebRunnerTest
```

Para ejecutar los escenarios enviando variables de ambiente

```bash
  ./gradlew clean test --tests "runners.automationtest.WebRunnerTest" -Dvariable1=test
```
...
> **NOTA**:
> * Antes de empezar a usar el arquetipo para un nuevo proyecto se debe elimiar todos los objetos demo:
...
[Manual SerenityBDD](https://dev.azure.com/BancoPichinchaEC/BP-Quality-Management/_wiki/wikis/BP-Quality-Management.wiki/601/Manual-Arquetipo-SerenityBDD-ScreenPlay)
